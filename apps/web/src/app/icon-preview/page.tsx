...
const PALETTE = [
  ...
  { key: "medieval",    label: "Medieval",    short: "Med", hex: "#90a0a8" },
  { key: "councils",    label: "Councils",    short: "Con", hex: "#60d4c8" },
];
...
export default function LoadingPreview() {
  ...
  const [dismissedLines, setDismissedLines] = useState<string[]>([]); // source→BoC lines dismissed after color pulse
  ...
  function reset() {
    ...
    setSourcesGone(false); setDismissedLines([]); setReturnLineDone(false); setBocGone(false); setBorderFlash(false); setResultsIn(false);
  }
  ...
  function startAnimation() {
    ...
    // ── STEP 1: search→BoC gold line draws; BoC fades in (1s) ────────
    at(() => { setPhase(1); setSearchLineDone(true); }, 100);

    // ── STEP 2: BoC opening pulse fires 300ms before line arrives ─────
    // Line math-arrives at 1100ms (100ms start + 1s draw)
    // 300ms early ≈ same visual point as 700ms/3s: line is ~70% through = ~80% drawn
    at(() => setOpeningPulse(true),  800);   // 300ms before 1100ms arrival
    at(() => setSearchGone(true),    1400);  // 300ms buffer (100ms was too tight at 1s)
    at(() => setOpeningPulse(false), 1800);  // 1000ms on-time

    // ── STEP 3: sources start appearing halfway through opening pulse fade-out
    // Pulse ends at 1800ms, fade-out is 1.0s → halfway = 1800 + 500 = 2300ms
    at(() => { setPhase(2); setSourcesReady(true); }, 2300);

    // ── STEP 4: source pulse fires 200ms before BoC→source lines arrive
    // Lines: start 2300ms + 1000ms → arrive 3300ms
    at(() => setGlowColor(true),  3100);
    at(() => setGlowColor(false), 4100);  // 1000ms single pulse

    // ── STEP 5: source→chunk gold lines draw (1s); 100ms hold after pulse
    at(() => setPhase(3), 4200);

    // ── STEP 6: winner chunk pulses (75ms after arrival) ──────────────
    // Lines: start 4200ms + 1000ms → arrive 5200ms
    at(() => setChunkFlash(true),  5275);
    at(() => setChunkFlash(false), 5775);
    at(() => setChunkFlash(true),  6175);
    at(() => setChunkFlash(false), 6675);

    // ── STEP 7: others fade; colored chunk→source lines draw (1s) ────
    at(() => setPhase(5), 6750);

    // ── STEP 8: source pulse fires 200ms before chunk→source lines arrive
    // Lines: start 6750ms + 1000ms → arrive 7750ms
    at(() => setGlowColor(true),  7550);
    at(() => setGlowColor(false), 8550);  // 1000ms single pulse

    // ── STEP 9: source→BoC colored lines draw (1s); 100ms hold after pulse
    at(() => setPhase(7), 8650);

    // ── STEP 10: BoC cycles through each source's color ──────────────
    // Lines: start 8650ms + 1000ms → arrive 9650ms; fire 100ms early
    const bocPulseStart = 9550;
    active.forEach((key, i) => {
      const t = bocPulseStart + i * 800;
      at(() => setQPulse(palMap[key].hex), t);
      at(() => { setQPulse(null); setDismissedLines(prev => [...prev, key]); }, t + 250);
    });

    // ── STEP 11: gold pulse — 1200ms on
    const goldT = bocPulseStart + active.length * 800;
    at(() => setQPulse(ACCENT), goldT);
    at(() => setQPulse(null),   goldT + 1200);

    // ── STEP 12: sources fade at goldT+800; return line draws once gold pulse ends
    at(() => setSourcesGone(true),    goldT + 800);
    at(() => setReturnLineDone(true), goldT + 1200);

    // ── STEP 13: flash fires 50ms before return line arrives ─────────
    // Return line starts goldT+1200, travels 1s → arrives goldT+2200
    at(() => { setBocGone(true); setBorderFlash(true); }, goldT + 2150);
    at(() => setBorderFlash(false), goldT + 3150);  // 1000ms flash

    // ── STEP 14: results appear as border flash begins fading ─────────
    at(() => setResultsIn(true), goldT + 3150);

    at(() => setPhase(9), goldT + 3650);
  }
        ...
        <svg width={W} height={H}>
          ...
          <line
            ...
            style={{
              ...
              transition: searchLineDone ? "stroke-dashoffset 1s linear" : "none",
            }}
          />
          ...
          {sources.map(s => {
            ...
            return (
              <g key={`src-${s.key}`}
                ...
                style={{ filter: sourcesReady && !sourcesGone ? filter : undefined, transition: `opacity ${sourcesGone ? "1.125s ease-in-out" : "0.8s ease-out"}, filter ${glowColor ? "0.8s ease-out" : "0.8s ease-in-out"}` }}
              >
