...
def _gen(n):
    return GenerationOutput.model_validate(
        {"facets": [{"text": f"working{i}", "takeaway": f"t{i}", "question": f"q{i}"}
                    for i in range(n)]})


def _cls(kinds, grounding="explicit"):
    return ClassificationOutput.model_validate(
        {"labels": [{"grounding": grounding, "evidence": f"e{i}", "kind": k}
                    for i, k in enumerate(kinds)]})


def test_merge_zips_parallel_arrays():
    m = merge(_gen(2), _cls(["doctrinal", "typological"]), "SUMMARY: x")
...
def test_merge_carries_grounding_and_evidence():
    m = merge(_gen(2), _cls(["doctrinal", "moral"], grounding="settled"), "SUMMARY: x")
...
def test_merge_carries_kind_secondary_when_present():
    cls = ClassificationOutput.model_validate(
        {"labels": [{"grounding": "explicit", "evidence": "e0", "kind": "doctrinal",
                     "kind_secondary": "moral"},
                    {"grounding": "explicit", "evidence": "e1", "kind": "moral"}]})
    m = merge(_gen(2), cls, "SUMMARY: x")
    assert m.facets[0].kind_secondary == "moral"


def test_merge_kind_secondary_defaults_to_none():
    m = merge(_gen(2), _cls(["doctrinal", "moral"]), "SUMMARY: x")
    assert m.facets[0].kind_secondary is None


def test_merge_length_mismatch_raises():
    with pytest.raises(MergeError):
        merge(_gen(3), _cls(["doctrinal", "typological"]), "SUMMARY: x")


def test_merge_too_few_facets_raises():
    with pytest.raises(MergeError):
        merge(_gen(1), _cls(["doctrinal"]), "SUMMARY: x")


def test_merge_too_many_facets_raises():
    with pytest.raises(MergeError):
        merge(_gen(13), _cls(["doctrinal"] * 13), "SUMMARY: x")
